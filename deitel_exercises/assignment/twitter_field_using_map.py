...
verified_users = list(map(lambda y: y["username"],filter(lambda x: x["verified"], fields)))
verified_users_alternative = list(filter(lambda x: x,map(lambda y: y["username"] if y["verified"] else False, fields)))
# active_users = map(field["username"] for field in fields for tweets in field["tweets"])
# users_below_25_and_verified = map(field["username"] for field in fields if field["age"] < 25 if field["verified"] == True)
# users_age = map(list(field["age"] for field in fields))
...
print(verified_users_alternative)
